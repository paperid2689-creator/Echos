...
MODEL_PATH = "/llms/Qwen3-VL-8B-Instruct/"

# Where to save results
RESULT_DIR = "Q3-5Res-17"
LABEL_TO_SUBFOLDER = {
    "A4C": "A4C",
    "SC": "SC",
    "PL": "PL",
...
# Sweep of few-shot ratios per class:
# 0.00 = 0% (true zero-shot), 0.01 = 1%, etc.
SHOT_RATIOS = [0.00, 0.001, 0.003, 0.005, 0.007]

# Fraction of remaining images to evaluate on (after removing few-shot examples)
SAMPLE_RATIO = 0.002  # 2% of remaining images

RANDOM_SEED = 42

# Debug: show per-label Yes/No scores for first few images (only for first shot_ratio)
DEBUG_SHOW_SCORES = True
DEBUG_NUM_IMAGES = 5

# Target image size for all images
...
def load_image_224(path, size=TARGET_IMAGE_SIZE):
    """Load image from disk and resize to 224x224 RGB."""
...
def collect_image_label_pairs():
    """
    Walk the six subfolders and collect (image_path, label) pairs.
    label ∈ {A4C, SC, PL, PSAV, PSMV, Random}.
    """
    image_label_pairs = []
...
def select_label_examples_by_ratio(image_label_pairs, shot_ratio_per_class: float):
    """
    Select few-shot examples as a percentage of available samples per label.

    For each label with N_c images:
        if shot_ratio_per_class == 0:
            shots_c = 0  (true zero-shot for that label)
        else:
            shots_c = max(1, int(shot_ratio_per_class * N_c))

    Returns:
      label_to_examples: dict[label] -> list[example_img_path]
      remaining_pairs:   list[(img_path, label)] excluding images used as examples
    """
...
def build_label_context_inputs(
    ...
    # Few-shot exemplars for this label (may be empty)
...
def predict_label_for_image(
    ...
    Works for both few-shot and pure zero-shot (when some example lists are empty).
    """
    best_label = None
    ...
    for label in LABELS:
        ...
        # Clear cache per-label to be safe with memory
...
def main():
    ...
    # 3) For each shot_ratio, select few-shot examples and evaluate
    for i, shot_ratio in enumerate(SHOT_RATIOS):
        ...
        n_remaining = len(remaining_pairs)
        print(
            f"Remaining images after few-shot selection "
            f"(shot_ratio={shot_ratio:.3f}): {n_remaining}"
        )
